...
public class Tiledcontrol1 : MonoBehaviour
{
    ...
    {
        if (collision.transform.tag == "Agit")
        {
            if (GameManager.GetInstance.myTurn)
            {
                tclickCheck = collision.transform.GetComponent<Unit>().clickCheck;
                if (tclickCheck)
                {
                    if (collision.transform.GetComponent<Unit>().attackCheck)
                    {
                        tattackRange = collision.transform.GetComponent<Unit>().attackRange;
                        collision.transform.GetComponent<BoxCollider2D>().size = new Vector2(158 * tattackRange, 214);
                        collision.transform.GetComponent<Unit>().enemyattackCheck = true;
                        StartCoroutine(CheckAttackPlayer());
                    }
                }
                if (!tclickCheck)
                {
                    collision.transform.GetComponent<BoxCollider2D>().size = new Vector2(158, 214);
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        ...
        else if(collision.transform.tag =="Agit")
        {
            if(collision.transform.GetComponent<Unit>().control_player == PLAYER.PLAYER1)
            {
                StartCoroutine(CheckExitPlayer());
                charOnoff = false;
                tiledUnitCheck = false;
            }
            else if (collision.transform.GetComponent<Unit>().control_player == PLAYER.PLAYER2)
            {
                StartCoroutine(CheckExitPlayer2());
                charOnoff2 = false;
                tiledUnitCheck = false;
